...
#----------------------------------------------------------------------------------------------------------
#' Create a cytoscape.js JSON graph from an igraph object
#'
...
igraphsToJSON <- function(g)
{
  tbl.edges <- igraph::as_data_frame(g)
  tbl.edges$interaction <- NA
  names(tbl.edges) <- c("source","target","interaction")
